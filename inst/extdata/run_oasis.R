...
files = get_image_filenames_list_by_subject(type="coregistered", group="training")
isubj = 1
oasis_dfs_train = list()
for (isubj in seq_along(files)) {
  ...
  if(!file.exists(outfile)){
    df = oasis_train_dataframe(flair = FLAIR, 
  ...
  oasis_dfs_train = list(oasis_dfs_train, df)
  rm(list="df")
}

  model = do.call("oasis_training", oasis_dfs_train)
  # paste(unlist(lapply(1:5, function(x) paste0("oasis_dfs[[",x,"]]"))), collapse=",")
